...
def get_data_dict(feature_dir_RGB, feature_dir_FLOW, label_dir, video_list, event_list, sample_rate=4, temporal_aug=True, boundary_smooth=None,proposal_dir=None):
    ...
    for video in tqdm(video_list):
        ...
        if proposal_dir is not None:
            ...
            # 没有 proposal 时，用 GT one-hot 兜底，方便代码兼容
            safe_label = event_seq_raw.astype(np.int64)
            safe_label = np.clip(safe_label, 0, len(event_list) - 1)
            proposal_prob_raw = np.eye(len(event_list), dtype=np.float32)[safe_label].T
