    ...
    for video in tqdm(video_list):
        ...
        if proposal_dir is not None:
            ...
            if proposal_prob_raw.shape[1] == len(event_list):
                ...
                proposal_prob_raw = proposal_prob_raw.T
            else:
                raise ValueError(
            ...
            if proposal_prob_raw.shape[0] != frame_num:
                raise ValueError(
                    f'Length mismatch in {video}: '
                    f'GT length={frame_num}, proposal length={proposal_prob_raw.shape[0]}'
                )

            proposal_prob_raw = np.maximum(proposal_prob_raw, 0.0)
            row_sum = proposal_prob_raw.sum(axis=1, keepdims=True)
            zero_rows = np.where(row_sum.squeeze(1) <= 1e-8)[0]

            if len(zero_rows) > 0:
                valid_rows = np.where(row_sum.squeeze(1) > 1e-8)[0]

                for r in zero_rows:
                    if len(valid_rows) == 0:
                        proposal_prob_raw[r] = 1.0 / len(event_list)
                    else:
                        nearest = valid_rows[np.argmin(np.abs(valid_rows - r))]
                        proposal_prob_raw[r] = proposal_prob_raw[nearest]

                row_sum = proposal_prob_raw.sum(axis=1, keepdims=True)

            proposal_prob_raw = proposal_prob_raw / np.maximum(row_sum, 1e-8)
            proposal_prob_raw = proposal_prob_raw.T
