...
def load_config_file(config_file):
    ...
    if 'use_east_proposal' not in all_params:
        all_params['use_east_proposal'] = False

    if 'proposal_alpha' not in all_params:
        all_params['proposal_alpha'] = 0.5

    if 'east_proposal_gpu_dir' not in all_params:
        all_params['east_proposal_gpu_dir'] = 'gpu2_id0'
...
def func_eval(label_dir, pred_dir, video_list):
    ...
    for vid in video_list:
        ...
        # --- 修复开始 ---
        # 旧代码: gt_content = read_file(gt_file).split('\n')[0:-1]
        # 这行代码假设文件末尾只有一个换行符，如果文件没有换行符或有多余空行，读取长度就会出错。
        # 使用 .split() 可以自动处理所有空白字符（空格、换行），这是最鲁棒的方法。
        ...
        # 如果长度不一致，打印调试信息以便定位问题视频
        if len(gt_content) != len(pred_content):
            print(f"Error in video {vid}: GT length {len(gt_content)}, Pred length {len(pred_content)}")
